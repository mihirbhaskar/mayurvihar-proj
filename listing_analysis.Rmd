...
# Exporting list of kids between 5-16
temp <- ind %>% select(hhid, block, name, gender, dob, age, aadhaar_num, resp_name, phone_num) %>% filter(age >= 5 & age <= 16)
temp <- temp %>% rename(parent_name = resp_name)

writexl::write_xlsx(temp, path = paste0(base,"kids_5to16.xlsx"))
